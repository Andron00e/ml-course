...
Original file is located at
    https://colab.research.google.com/drive/1-3wLf4J-QfP4pYASEml7vEK5xuk77fcE
...
class SoftMax(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        prox = np.diag(exp.dot(gradOutput.T))
        self.gradInput = - np.diag(prox).dot(exp)
...
class LogSoftMax(Module):
    ...
    def updateOutput(self, input):
        ...
        self.sm_output = exp / exp.sum(axis=1, keepdims=True)
        self.output = np.log(self.sm_output)
        return self.output

    def updateGradInput(self, input, gradOutput):
        # Your code goes here. ################################################
        self.gradInput = gradOutput - self.sm_output
        return self.gradInput
...
class BatchNormalization(Module):
    ...
    def updateOutput(self, input):
        ...
        batch_variance = np.std(input, axis=0, keepdims=True)
        self.output = input
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        denom = -1 / 2 / (batch_variance ** 2 + self.EPS) * np.sum(np.multiply(gradOutput, input - batch_mean), axis=0)
        self.gradInput = gradOutput + (2*np.multiply(input - batch_mean, denom.reshape(1, input.shape[1])) - np.sum(gradOutput, axis=0))  / input.shape[0]
...
class ClassNLLCriterionUnstable(Criterion):
    ...
    def updateOutput(self, input, target):
        ...
        # Your code goes here. ################################################
        self.output = -1 / input.shape[0] * (target * np.log(input_clamp)).sum()
        return self.output
...
class Conv2d(Module):
    ...
    def updateOutput(self, input):
        ...
        for i in range(input.shape[0]):
            padded = np.pad(input[i], ((0, 0), (pad_size, pad_size), (pad_size, pad_size)))
            for j in range(self.out_channels):
                self.output[i][j] = scipy.signal.correlate(padded, self.W[j], mode='valid') + self.b[j]
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        for i in range(input.shape[0]):
            padded = np.pad(gradOutput[i], ((0, 0), (pad_size, pad_size), (pad_size, pad_size)))
            for j in range(self.in_channels):
                self.gradInput[i][j] = scipy.signal.correlate(padded, np.flip(self.W[:, j, ...], axis=(1,2)), mode='valid')
    ...
    def accGradParameters(self, input, gradOutput):
        ...
        padded = np.pad(input, ((0,0), (0,0), (pad_size, pad_size), (pad_size, pad_size)))
        # self.gradW = ...
        # self.gradb = ...
        self.gradW = np.zeros_like(self.W)
        ...
        for i in range(self.out_channels):
            for j in range(self.in_channels):
                self.gradW[i][j] = scipy.signal.correlate(padded[:, j, ...], gradOutput[:, i, ...], mode='valid')
...
class MaxPool2d(Module):
    ...
    def updateOutput(self, input):
        ...
        # self.output = ...
        # self.max_indices = ...
        reshaped = np.reshape(input, (input.shape[0], input.shape[1],
                                     input.shape[2] // self.kernel_size, self.kernel_size,
                                     input.shape[3] // self.kernel_size, self.kernel_size))
        swapped = np.swapaxes(reshaped, 3, 4)
        self.output = np.max(swapped, axis=(4,5))
        self.max_indices = input == np.reshape(np.swapaxes(np.max(swapped, axis=(4,5), keepdims=True) +
                                    np.zeros_like(swapped), 3,4), input.shape)
        return self.output

    def updateGradInput(self, input, gradOutput):
        # YOUR CODE #############################
        # self.gradInput = ...
        grad = np.reshape(gradOutput, (gradOutput.shape[0], gradOutput.shape[1],
                                           gradOutput.shape[2], gradOutput.shape[3], 1, 1))
        grad = grad * np.ones((gradOutput.shape[0], gradOutput.shape[1],
                                           gradOutput.shape[2], gradOutput.shape[3], self.kernel_size, self.kernel_size))
        self.gradInput = np.reshape(np.swapaxes(grad, 3, 4), input.shape)
